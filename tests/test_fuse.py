...
def test_create_trace(fuse_client):
    trace = fuse_client.create_trace(f"trace_id-{timestamp()}", "trace_name_1", "session_id_1", "user_id_123")
...
def test_create_span(fuse_client):
    trace = fuse_client.create_trace(f"trace_id-{timestamp()}", "trace_name_x", "session_id_2", "user_id_123")
...
def test_create_generation(fuse_client):
    trace = fuse_client.create_trace(f"trace_id-{timestamp()}", "trace_name_3", "session_id_3", "user_id_123")

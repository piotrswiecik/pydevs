from pydevs.types.completion import TextCompletionPayload
from pydevs.services.base import AIServiceBase

def preview_image_system_message(
    ai: AIServiceBase, title: str, text: str, extr_type: str, extr_des: str
):
    return TextCompletionPayload(
        content="""Generate a brief, factual description of the provided image based solely on its visual content.

You will analyze the image and produce a concise description that captures its essential visual elements without any additional context, returning it in JSON format.
...
def refine_description_system_message(ai: AIServiceBase) -> TextCompletionPayload:
    return TextCompletionPayload(
...
def extract_image_context_system_message(ai: AIServiceBase, images: list) -> TextCompletionPayload:
...
Upon receiving an article, analyze it to extract context for any mentioned images, creating an array of JSON objects as demonstrated. Adhere strictly to the provided rules, focusing solely on explicitly stated image details within the text.""".format(image_list),
        role="system"
    )